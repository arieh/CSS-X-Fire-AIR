...
var params = {
	...
	, handleCSS : function handleCSS(data,content){
		var esSelector = data.selector.escapeRegExp().replace(/\s/,'\\s*')
		  , pattern = '((?:^|})\\s*)('+esSelector+'\\s*\\{[\\S\\s]*)(?:'+data.property.escapeRegExp()+'\\s*:)(?:[^;\\}]+)(;|\\})?(?![\\s\\S]*(?:'+esSelector+'\\s*\\{[\\S\\s]*)(?:'+data.property.escapeRegExp()+'\\s*:))'
		  , regex = new RegExp(pattern,'xi')
		  , replace = (data.deleted === 'true') ? ('$1$3') : ("$1$2" + data.property + " : " + data.value + "$3")
